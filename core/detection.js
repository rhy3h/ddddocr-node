 ...
 */
function _preProcessImage(image, inputSize) {
 ...
 */
function _demoPostProcess(cpuData, dims, imageSize) {
    const grids = [];
 ...
 */
function _calcBbox(boxes, ratio) {
 ...
 */
function _getCurrentBox(boxes, orders = undefined) {
    if (orders == undefined) {
 ...
 */
function _nms(boxes, scores, nmsThr) {
    let order = argSort(scores);
    const keep = [];

    const [x1, y1, x2, y2] = _getCurrentBox(boxes);
    ...
    while (order.length > 0) {
        ...
        const [x1, y1, x2, y2] = _getCurrentBox(boxes, order);
        ...
        const inds = _where(ovr.arraySync(), nmsThr);
 ...
 */
function _multiclassNmsClassAgnostic(boxes, scores, nmsThr, scoreThr) {
    ...
    const keep = _nms(validBoxes, validScores, nmsThr);
 ...
 * @returns {Array<Array<number>>} An array of bounding boxes after NMS, where each box is represented as [x1, y1, x2, y2].
 */
function _multiclassNms(boxes, scores, nmsThr, scoreThr) {
    return _multiclassNmsClassAgnostic(boxes, scores, nmsThr, scoreThr);
}
...
class Detection {
    ...
    async _getBbox(image) {
        ...
        const { inputArray, ratio } = _preProcessImage(image, inputSize);
        ...
        const predictions = _demoPostProcess(cpuData, dims, inputSize);

        const boxes = predictions.slice([0, 0], [-1, 4]);
        const scores = predictions.slice([0, 4], [-1, 1]).mul(predictions.slice([0, 5], [-1, 1]));

        const boxesXyxy = _calcBbox(boxes, ratio);

        const prediction = _multiclassNms(boxesXyxy, scores, 0.45, 0.1);
